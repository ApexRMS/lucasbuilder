...
### Transition type
distTypes <- read_xlsx(
  path = paste0(initialInputsDirectory, "Disturbance Type.xlsx"),
  sheet = "Disturbance Type"
) %>%
  data.frame() %>%
  ...
  mutate(
    color = case_when(
      str_detect(Name, "fire") ~ "255,255,0,0",
      str_detect(Name, "cut") ~ "255,255,255,0"
    )
  )

transitionTypes <- distTypes %>% mutate(transitions = paste0(Name, " [Type]"))
...
mySheet <- datasheet(myProject, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "Age Groups.xlsx"),
  sheet = "Age Groups"
) %>%
  data.frame()
saveDatasheet(myProject, mySheetFull, sheetName)
...
mySheet <- datasheet(myProject, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "Attribute Group.xlsx"),
...
mySheet <- datasheet(myProject, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "State Attribute Type.xlsx"),
  sheet = "State Attribute Type"
) %>%
  data.frame()
...
mySheet <- datasheet(myProject, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "Stock Type.xlsx"),
  sheet = "Stock Type"
) %>%
  data.frame()
saveDatasheet(myProject, mySheetFull, sheetName)
...
mySheet <- datasheet(myProject, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "Stock Group.xlsx"),
  sheet = "Stock Group"
) %>%
  data.frame()
saveDatasheet(myProject, mySheetFull, sheetName)
...
mySheet <- datasheet(myProject, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "Flow Type.xlsx"),
  sheet = "Flow Type"
) %>%
  data.frame()
saveDatasheet(myProject, mySheetFull, sheetName)
...
mySheet <- datasheet(myProject, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "Flow Group.xlsx"),
  sheet = "Flow Group"
) %>%
  data.frame()
saveDatasheet(myProject, mySheetFull, sheetName)
...
mySheet <- datasheet(myProject, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "Ecological Boundary.xlsx"),
  sheet = "Ecological Boundary"
) %>%
  data.frame()
saveDatasheet(myProject, mySheetFull, sheetName)
...
mySheet <- datasheet(myProject, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "Administrative Boundary.xlsx"),
  sheet = "Administrative Boundary"
) %>%
  data.frame()
saveDatasheet(myProject, mySheetFull, sheetName)
...
mySheet <- datasheet(myProject, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "Species Type.xlsx"),
  sheet = "Species Type"
) %>%
  data.frame()
saveDatasheet(myProject, mySheetFull, sheetName)
...
mySheet <- datasheet(myProject, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "CBM-CFS3 Crosswalk Carbon Stock.xlsx"),
  sheet = "CBM-CFS3 Crosswalk Carbon Stock"
) %>%
  data.frame() %>%
...
myScenario = scenario(
  myProject,
  scenario = myScenarioName,
  folder = "1 - Predefined Inputs"
)

sheetName <- "stsim_FlowPathwayDiagram"
mySheet <- datasheet(myScenario, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "Flow Pathway Diagram.xlsx"),
  sheet = "Flow Pathway Diagram"
) %>%
  data.frame()
names(mySheetFull) <- names(mySheet)
...
mySheet <- datasheet(myScenario, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "Flow Pathways.xlsx"),
  sheet = "Flow Pathways"
) %>%
  data.frame()
names(mySheetFull) <- names(mySheet)
...
mySheet <- datasheet(myScenario, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "Flow Order.xlsx"),
  sheet = "Flow Order"
) %>%
  data.frame()
names(mySheetFull) <- names(mySheet)
...
mySheetFull <- data.frame(
  ApplyBeforeTransitions = T,
  ApplyEquallyRankedSimultaneously = T
)
saveDatasheet(myScenario, mySheetFull, sheetName)
...
mySheet <- datasheet(myScenario, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "Flow Type-Group Membership.xlsx"),
  sheet = "Flow Type-Group Membership"
) %>%
  data.frame() %>%
  mutate(
    Flow.Type = str_replace(Flow.Type, "Net Growth:", "Net Growth Forest:")
  )
names(mySheetFull) <- names(mySheet)
saveDatasheet(myScenario, mySheetFull, sheetName)
...
mySheet <- datasheet(myScenario, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "Stock Type-Group Membership.xlsx"),
  sheet = "Stock Type-Group Membership"
) %>%
  data.frame()
names(mySheetFull) <- names(mySheet)
saveDatasheet(myScenario, mySheetFull, sheetName)
...
mySheet <- datasheet(myScenario, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "Initial Stocks - Non Spatial.xlsx"),
  sheet = "Initial Stock - Non Spatial"
) %>%
  data.frame()
names(mySheetFull) <- names(mySheet)
saveDatasheet(myScenario, mySheetFull, sheetName)
...
mySheet <- datasheet(myScenario, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "SF Output Options - Spatial.xlsx"),
  sheet = "SF Output Options"
) %>%
  data.frame()
# names(mySheetFull) <- names(mySheet)
saveDatasheet(myScenario, mySheetFull, sheetName)
...
mySheet <- datasheet(myScenario, name = sheetName, optional = T, empty = T)
mySheetFull <- transitionTypes %>% select(Name, transitions)
names(mySheetFull) <- names(mySheet)
saveDatasheet(myScenario, mySheetFull, sheetName)
...
myScenario <- scenario(
  myProject,
  scenario = "Output Options [Spatial]",
  folder = "1 - Predefined Inputs"
)
sheetName <- "stsim_OutputOptionsSpatial"
mySheet <- datasheet(myScenario, name = sheetName, empty = T, optional = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "Output Options Spatial.xlsx"),
  sheet = "Output Options Spatial"
) %>%
  data.frame()
names(mySheetFull) <- names(mySheet)
saveDatasheet(myScenario, mySheetFull, sheetName)
...
folder2.1 <- folder(
  ...
  folder = "2.1 - Run Setup Inputs",
  parentFolder = folder2
)
folder2.2 <- folder(
  ssimObject = myProject,
  folder = "2.2 - Run Forecast Inputs",
  parentFolder = folder2
)
...
crosswalkSUSTFull$CBMSimulationDataFilePath <- paste0(
  CBMDir,
  crosswalkSUSTFull$CBMSimulationDataFileName
)
...
for (i in seq(1:nrow(crosswalkSUSTFull))) {
  ...
  mySheet = addRow(mySheet, data.frame(Name = myValue, Id = 1))
...
mySheet <- datasheet(myProject, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(initialInputsDirectory, "user-example-inputs/State Class.xlsx"),
  sheet = "State Class"
) %>%
  data.frame()
names(mySheetFull) <- names(mySheet)
...
sheetName <- "stsim_AgeGroup"
mySheet <- datasheet(myProject, name = sheetName, optional = T)
mySheet[1:(maxAge / 20), "MaximumAge"] <- c(
  seq(from = 20, to = (maxAge - 1), by = 20),
  maxAge - 1
)
saveDatasheet(myProject, mySheet, name = sheetName)
...
myScenario <- scenario(
  ...
  folder = "2.1 - Run Setup Inputs"
) # <- paste0("Run Control [Non-spatial; ", maxTimestep, " years; ", maxIteration, " MC]"))
...
myScenario <- scenario(
  myProject,
  scenario = "Run Control - Landscape",
  folder = "2.2 - Run Forecast Inputs"
)
sheetName <- "stsim_RunControl"
mySheet <- datasheet(myScenario, name = sheetName, empty = T, optional = T)
...
mySheetFull <- data.frame(
  StratumFileName = paste0(
    path.expand(initialInputsDirectory),
    "user-example-inputs/spatial/eco-boundary.tif"
  ),
  SecondaryStratumFileName = paste0(
    path.expand(initialInputsDirectory),
    "user-example-inputs/spatial/admin-boundary.tif"
  ),
  StateClassFileName = paste0(
    path.expand(initialInputsDirectory),
    "user-example-inputs/spatial/state-class.tif"
  ),
  AgeFileName = paste0(
    path.expand(initialInputsDirectory),
    "user-example-inputs/spatial/age.tif"
  )
)
saveDatasheet(myScenario, mySheetFull, sheetName)
...
mySheetFull <- data.frame(
  TransitionGroupId = "Wildfire [Type]",
  TSTFileName = paste0(
    path.expand(initialInputsDirectory),
    "user-example-inputs/spatial/age.tif"
  )
...
myScenario <- scenario(
  myProject,
  scenario <- "Transition Pathways - Landscape",
  folder = "2.2 - Run Forecast Inputs"
)
sheetName <- "stsim_DeterministicTransition"
mySheet <- datasheet(myScenario, name = sheetName, optional = T, empty = T)
...
myScenario <- scenario(
  myProject,
  scenario <- "Transition Multiplier - Landscape",
  folder = "2.2 - Run Forecast Inputs"
)
...
mySheet = data.frame(
  TransitionGroupId = c("Wildfire [Type]", "Wildfire [Type]"),
  MaximumArea = c(90, 80),
  RelativeAmount = c(0.9, 0.1)
)
mySheetFull = bind_rows(mySheetFull, mySheet)
...
# mySheet <- datasheet(myScenario, name=sheetName, optional=T, empty = T)
mySheetFull <- data.frame(StageNameId = "1 - Load CBM Output", RunOrder = 1)
saveDatasheet(myScenario, mySheetFull, sheetName)
...
mySheetFull <- data.frame(
  StageNameId = c(
    "3 Spin-up Pre-Process (before ST-Sim)",
    "ST-Sim",
