...
mySession <- session()
libraryName <- paste0(getwd(), "/model/lucasbuilder-spatial")
myProjectName <- "Definitions"
initialInputsDirectory <- paste0(
  dirname(getwd()),
  "/data/user-example-inputs-merch/"
)
definitionsPath <- paste0(initialInputsDirectory, "ConusLibrary/")


# Spatial data ----
adminRast <- rast(file.path(
...
mySheetFull <- read_xlsx(
  path = paste0(dirname(getwd()), "/data/Age Groups.xlsx"),
  sheet = "Age Groups"
) %>%
  data.frame()
saveDatasheet(myProject, mySheetFull, sheetName)
...
mySheet <- datasheet(myProject, name = sheetName, optional = T, empty = T)
mySheetFull <- read.csv(paste0(
  definitionsPath,
  "Ecological Boundary LUCAS.csv"
))
saveDatasheet(myProject, mySheetFull, sheetName)
...
mySheetFull <- read.csv(paste0(
  definitionsPath,
  "Administrative Boundary LUCAS.csv"
))
saveDatasheet(myProject, mySheetFull, sheetName)
...
mySheetFull <- read.csv(paste0(
  definitionsPath,
  "CBM Crosswalk Carbon Stock.csv"
)) %>%
  select(CBMStock) %>%
...
mySheetFull <- read.csv(paste0(
  definitionsPath,
  "Flow Type-Group Membership.csv"
))
names(mySheetFull) <- gsub("ID", "Id", names(mySheetFull))
saveDatasheet(myScenario, mySheetFull, sheetName)
...
mySheetFull <- read.csv(paste0(
  definitionsPath,
  "Stock Type-Group Membership.csv"
))
names(mySheetFull) <- gsub("ID", "Id", names(mySheetFull))
saveDatasheet(myScenario, mySheetFull, sheetName)
...
mySheet <- datasheet(myScenario, name = sheetName, optional = T, empty = T)
mySheetFull <- read_xlsx(
  path = paste0(dirname(getwd()), "/data/Stock-Flow Output Options.xlsx"),
  sheet = "Stock-Flow Output"
) %>%
  data.frame()
...
crosswalkSUSTPath <- paste0(
  initialInputsDirectory,
  "Crosswalk - Spatial Unit and Species Type.csv"
)

crosswalkSUSTFull <- read.csv(crosswalkSUSTPath, check.names = F)
crosswalkSUSTFull$CBMOutputFile <- paste0(
  CBMDir,
  crosswalkSUSTFull$CBMOutputFile
)
...
mySheetFull <- read.csv(paste0(
  initialInputsDirectory,
  "MerchantableVolumeCurves.csv"
))
mySheetFull2 <- read.csv(paste0(
  initialInputsDirectory,
  "MerchantableVolumeCurvesRedWood.csv"
))
...
mySheetFull <- mySheetFull %>%
  ...
  addRow(mySheetFull2) %>%
  mutate(
    StratumId = as.character(unique(crosswalkSUSTFull$StratumId)),
    SecondaryStratumId = as.character(unique(
      crosswalkSUSTFull$SecondaryStratumId
    ))
  ) %>%
  arrange(Age) %>%
...
for (i in seq(1:nrow(crosswalkSUSTFull))) {
  crosswalkSUST <- crosswalkSUSTFull %>% slice(i)
  mySheet <- data.frame(
    StratumId = crosswalkSUST$StratumId,
...
for (i in seq(1:nrow(crosswalkSUSTFull))) {
  ...
  mySheet = data.frame(
    StateClassIdSource = as.character(myValue),
...
mySheet <- data.frame(StageNameId = "2 - Generate Flow Pathways", RunOrder = 1)
saveDatasheet(myScenario, mySheet, sheetName)
...
# set "spin-up" transformer to run for this scenario
sheetName <- "core_Pipeline"
mySheetFull <- data.frame(StageNameId = c("ST-Sim"), RunOrder = c(1))
saveDatasheet(myScenario, mySheetFull, sheetName)
