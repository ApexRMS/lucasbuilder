...
stratum_check <- spinup %>%
  ...
  summarize(
    Valid = ifelse(
      sum(StratumIsSpecified) == 0 | sum(StratumIsSpecified) == nrow(spinup),
      TRUE,
      FALSE
    )
  )
if (!stratum_check$Valid) {
  stop(
    "The ST-Sim Stratum in the Spin-up datasheet has both specified and unspecified values."
  )
}
...
secondary_stratum_check <- spinup %>%
  mutate(
    SecondaryStratumIsSpecified = ifelse(is.na(SecondaryStratumId), 0, 1)
  ) %>%
  summarize(
    Valid = ifelse(
      sum(SecondaryStratumIsSpecified) == 0 |
        sum(SecondaryStratumIsSpecified) == nrow(spinup),
      TRUE,
      FALSE
    )
  )
if (!secondary_stratum_check$Valid) {
  stop(
    "The ST-Sim Secondary Stratum in the Spin-up datasheet has both specified and unspecified values."
  )
}
...
saveDatasheet(myProject, data = unique_tertiary, name = "stsim_TertiaryStratum")

# save updated spinup
saveDatasheet(myScenario, data = spinup, name = "lucasbuilder_Spinup")
...
the_end <- spinup %>%
  mutate(
    EndPerSpinupCombination = SpinupDuration *
      ReturnInterval +
      MaxAgeForLastCycle
  ) %>%
...
for (rownb in 1:nrow_unique) {
  ...
  # Bind them
  temp_df <- bind_rows(
    data.frame(Timestep = ts_seq, Amount = 1),
  ...
  if (dist_hist != dist_last) {
    end_rows <- which(
      temp_df$Timestep %in% c(spinup_duration, spinup_duration + 1)
    )
  ...
  allTransitionGroups = datasheet(
    myProject,
    name = "stsim_TransitionGroup",
    returnInvisible = TRUE
  )
  excludeTransitionGroups = filter(
    allTransitionGroups,
    Name != includedTransGroups & IsAuto == TRUE
  )
...
final_df <- final_df %>% arrange(Timestep) %>% unique()
...
IC_nonspatial <- data.frame(
  TotalAmount = nrow_unique,
...
if (nrow(deter_transitions) == 0) {
  stop(
    "Deterministic Transitions datasheet is empty, transitions pathways could not be set"
  )
}

all_disturbances <- strip_type(unique(c(
  spinup_unique$MostRecentDisturbanceTGId,
  spinup_unique$HistoricalDisturbanceTGId
)))

new_transitions <- deter_transitions %>%
  select(-Location) %>%
  expand_grid(TransitionTypeId = all_disturbances, Probability = 1) %>%
  mutate_if(is.factor, as.character) %>%
  as.data.frame()

transitions_final <- bind_rows(transitions, new_transitions) %>% unique()
