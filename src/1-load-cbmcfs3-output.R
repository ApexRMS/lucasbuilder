...
stateAttributeInitialCarbonBiomass <- datasheet(
  myScenario,
  name = sheetName,
  empty = T,
  optional = T,
...
for (i in seq(1:nrow(crosswalkSUSTFull))) {
  ...
  if (!is.na(crosswalkSUST$CBMOutputFile)) {
    CBMSimulationData <- read.csv(
      crosswalkSUST$CBMOutputFile,
      header = TRUE,
      check.names = F
    )
    ...
    CBMSimulationData <- CBMSimulationData[
      CBMSimulationData$`Time Step` <= maxTimestep,
    ]

    # Get Species and Forest Type Ids
    speciesTypeId <- speciesTypeTable$SpeciesTypeID[
      speciesTypeTable$SpeciesTypeName ==
        as.character(crosswalkSUST$SpeciesTypeId)
    ]
    ...
    stateAttributeInitialCarbonBiomass[
      1:(nrow(CBMSimulationData) * numBiomassStocks),
      "StratumId"
    ] <- as.character(crosswalkSUST$StratumId)
    stateAttributeInitialCarbonBiomass[
      1:(nrow(CBMSimulationData) * numBiomassStocks),
      "SecondaryStratumId"
    ] <- as.character(crosswalkSUST$SecondaryStratumId)
    stateAttributeInitialCarbonBiomass[
      1:(nrow(CBMSimulationData) * numBiomassStocks),
      "StateClassId"
    ] <- as.character(crosswalkSUST$StateClassId)
    stateAtts <- NULL
    for (r in biomassStateAtts) {
      stateAtts <- c(stateAtts, rep(r, nrow(CBMSimulationData)))
    }
    stateAttributeInitialCarbonBiomass[
      1:(nrow(CBMSimulationData) * numBiomassStocks),
    ...
    stateAttributeInitialCarbonBiomass[
      1:(nrow(CBMSimulationData) * numBiomassStocks),
      "AgeMin"
    ] <- rep(CBMSimulationData[, "Time Step"], numBiomassStocks)
    stateAttributeInitialCarbonBiomass[
      1:(nrow(CBMSimulationData) * numBiomassStocks),
      "AgeMax"
    ] <- rep(CBMSimulationData[, "Time Step"], numBiomassStocks)
    stateAttributeInitialCarbonBiomass[
      stateAttributeInitialCarbonBiomass$AgeMin ==
        (nrow(CBMSimulationData) - 1),
      "AgeMax"
    ] <- NA
    value <- NULL
    for (r in biomassStocks) {
      value <- c(value, CBMSimulationData[, paste(forestType, r)])
    }
    stateAttributeInitialCarbonBiomass[
      1:(nrow(CBMSimulationData) * numBiomassStocks),
      "Value"
    ] <- value
    stateAttributeInitialCarbonBiomassFull = rbind(
      stateAttributeInitialCarbonBiomass,
      stateAttributeInitialCarbonBiomassFull
    )
  }
}

## State Attributes for DOM
stateAttributeInitialCarbonDOM <- datasheet(
  myScenario,
  name = sheetName,
  empty = T,
  optional = T,
...
for (i in seq(1:nrow(crosswalkSUSTFull))) {
  ...
  if (!is.na(crosswalkSUST$CBMOutputFile)) {
    ...
    stateAttributeInitialCarbonDOM[
      1:(nrow(CBMSimulationData) * numDOMStocks),
      "StratumId"
    ] <- as.character(crosswalkSUST$StratumId)
    stateAttributeInitialCarbonDOM[
      1:(nrow(CBMSimulationData) * numDOMStocks),
    ...
    stateAttributeInitialCarbonDOM[
      1:(nrow(CBMSimulationData) * numDOMStocks),
      "AgeMin"
    ] <- rep(CBMSimulationData[, "Time Step"], numDOMStocks)
    stateAttributeInitialCarbonDOM[
      1:(nrow(CBMSimulationData) * numDOMStocks),
      "AgeMax"
    ] <- rep(CBMSimulationData[, "Time Step"], numDOMStocks)
    stateAttributeInitialCarbonDOM[
      stateAttributeInitialCarbonDOM$AgeMin == (nrow(CBMSimulationData) - 1),
      "AgeMax"
    ] <- NA
    value <- NULL
    if (forestType == "Hardwood") {
      for (r in DOMStocks_hw) {
        value <- c(value, CBMSimulationData[, r])
      }
    }
    if (forestType == "Softwood") {
      for (r in DOMStocks_sw) {
        value <- c(value, CBMSimulationData[, r])
      }
    }
    stateAttributeInitialCarbonDOM[
    ...
    stateAttributeInitialCarbonDOMFull = rbind(
      stateAttributeInitialCarbonDOM,
      stateAttributeInitialCarbonDOMFull
    )
  }
}

## Combine the state attribute datasheets
stateAttributesMerged <- rbind(
  stateAttributeInitialCarbonBiomassFull,
  stateAttributeInitialCarbonDOMFull
)
...
for (row in 1:nrow(crosswalkSUSTFull)) {
  ...
  speciesTypeId <- speciesTypeTable$SpeciesTypeID[
    speciesTypeTable$SpeciesTypeName ==
      as.character(crosswalkSUST[1, "SpeciesTypeId"])
  ]
  forestTypeId <- speciesTypeTable$ForestTypeID[
    speciesTypeTable$SpeciesTypeID == speciesTypeId
  ]
  ...
  if (!is.na(crosswalkSUST$CBMOutputFile)) {
    ...
    # Remove time steps above run control maxTimestep
    validationDataWide <- validationDataWide[
      validationDataWide$`Time Step` <= maxTimestep,
    ]

    # convert validation carbon data from wide to long format
    if (forestType == "Softwood") {
      validationStocks <- CBM_Stocks_SW
    }
    if (forestType == "Hardwood") {
      validationStocks <- CBM_Stocks_HW
    }
    validationDataWide <- cbind(
      "Timestep" = validationDataWide[, "Time Step"],
      validationDataWide[, names(validationDataWide) %in% validationStocks]
    )
    validationCarbon <- gather(
      validationDataWide,
      Name,
      Amount,
      names(validationDataWide)[2:ncol(validationDataWide)],
      factor_key = TRUE
    )
    ...
    if (is.na(crosswalkSUST$StratumId)) {
      validationCarbon$StratumId <- "[Unspecified]"
    } else {
      validationCarbon$StratumId <- crosswalkSUST$StratumId
    }
    ...
    names(validationCarbon)[which(
      names(validationCarbon) == "Name"
    )] <- "StockGroupId"
...
SF_OutputStock <- datasheet(
  myScenario,
  name = "stsim_OutputStock",
  optional = T,
  lookupsAsFactors = F
)
